            ...
            <ul class="space-y-4">
                <li v-for="education in educations" :key="education.id" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                    ...
                    <h3 class="font-semibold">
                        <span class="text-sm text-gray-600 dark:text-green-400">
                                {{ new Date(education.date).toLocaleDateString() }}
                            </span> - {{ education.school }}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400" v-html="education.graduation"></p>
